...
def add_noise(z, sigma_noise=0.1):
    noise = torch.normal(0, sigma_noise, z.shape).cuda()
    return z + noise

# D_train function with bCR and zCR regularization
def D_train(x, G, D, D_optimizer, criterion, noise_factor=0.1, lambda_real=10, lambda_fake=10, lambda_dis=5):
    D.zero_grad()
...
def G_train(x, G, D, G_optimizer, criterion, noise_factor=0.1, lambda_gen=0.5):
    G.zero_grad()
